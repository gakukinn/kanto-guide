...
/**
 * WalkerPlus祭典四层详情页面模板 - 基于UniversalStaticDetailTemplate.tsx
...
// WalkerPlus祭典数据接口 - 对应14项字段（参照花火模板结构）
interface WalkerPlusMatsuriData {
  // 14项WalkerPlus祭典数据字段
  name: string;                // 1. 大会名
  reservationSystem: string;   // 2. 预约制
  viewingPoints: string;       // 3. 观赏点
...
interface WalkerPlusMatsuriTemplateProps {
  data: WalkerPlusMatsuriData;
  regionKey: string;
  activityKey: string;
...
export default function WalkerPlusMatsuriTemplate({
  data,
  regionKey,
  activityKey,
  regionConfig,
  activityConfig,
}: WalkerPlusMatsuriTemplateProps) {
  
  // 添加翻译hooks
  const t = useTranslations('matsuri');
  ...
  const activity = activityConfig || defaultActivityConfigs[activityKey] || defaultActivityConfigs.matsuri;
      ...
      <main className="relative z-10">
        ...
        <section className={`bg-gradient-to-r ${getStandardBackgroundGradient()} pb-8 pt-8`}>
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            {/* 图片展示卡片 - 与标题卡片样式一致 */}
            <div className={`mb-12 transform rounded-3xl border-2 border-red-200 bg-gradient-to-r ${getStandardBackgroundGradient()} p-1 md:p-4 shadow-xl backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:scale-[1.02] hover:shadow-2xl`}>
              ...
              <div className={`transform rounded-3xl border-2 border-red-200 bg-gradient-to-r ${getStandardBackgroundGradient()} p-6 shadow-xl backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:scale-[1.02] hover:shadow-2xl`}>
                ...
                <div className="space-y-4 text-base">
                      ...
                      <div className="font-bold text-gray-900">
                        {data.reservationSystem || tCommon('seeOfficial')}
                      </div>
                    </div>
                      ...
                      <div className="font-bold text-gray-900">
                        {data.viewingPoints || tCommon('seeOfficial')}
                      </div>
                    </div>
