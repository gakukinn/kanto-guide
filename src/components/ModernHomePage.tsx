      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute left-0 top-0 w-1/2 h-full overflow-hidden">
          ...
          {[...Array(30)].map((_, i) => (
            <div
              key={`sakura-${i}`}
              className="absolute text-pink-400 opacity-60 animate-pulse"
              style={{
                left: `${Math.random() * 80}%`,
                top: `${Math.random() * 100}%`,
          ...
          {/* 樱花树枝装饰 */}
          <div className="absolute top-10 left-10 text-6xl text-pink-300 opacity-40">🌸</div>
          <div className="absolute top-32 left-32 text-8xl text-pink-300 opacity-30">🌸</div>
          <div className="absolute bottom-20 left-16 text-7xl text-pink-300 opacity-35">🌸</div>
          
          {/* 粉色光晕 */}
          <div className="absolute top-1/3 left-1/4 w-80 h-80 bg-pink-300/20 rounded-full blur-3xl animate-pulse" />
        </div>
        
        {/* 右侧富士山装饰 */}
        <div className="absolute right-0 top-0 w-1/2 h-full overflow-hidden">
          {/* 富士山轮廓 */}
          <div className="absolute bottom-0 right-10 text-9xl text-blue-300 opacity-40">🗻</div>
          <div className="absolute bottom-10 right-32 text-7xl text-blue-400 opacity-35">⛰️</div>
          <div className="absolute top-20 right-20 text-6xl text-blue-300 opacity-30">🏔️</div>
          ...
          {/* 蓝色光晕 */}
          <div className="absolute top-1/4 right-1/4 w-80 h-80 bg-blue-300/20 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }} />
        </div>
            ...
            <div className="geographic-grid">
              {Object.entries(modernRegionConfig).map(([regionKey, region]) => (
                <Link
                  key={regionKey}
                  href={`/${regionKey}`}
                  className="group relative block cursor-pointer transform transition-all duration-700 hover:scale-105 hover:-translate-y-4"
                  style={{ gridArea: region.gridArea }}
                >
                                     {/* 主卡片容器 */}
                   <div className="relative h-full overflow-hidden rounded-3xl bg-white/10 backdrop-blur-xl border border-white/20 shadow-2xl">
                    
                    {/* 渐变背景 */}
                    <div className={`absolute inset-0 bg-gradient-to-br ${region.bgPattern} opacity-50`} />
                    
                    {/* 动态边框 */}
                    <div className={`absolute inset-0 rounded-3xl bg-gradient-to-r ${region.color} opacity-0 group-hover:opacity-100 transition-all duration-500 p-0.5`}>
                      <div className="w-full h-full rounded-3xl bg-black/20 backdrop-blur-xl" />
                    </div>
                    
                    {/* 光效装饰 */}
                    <div className={`absolute top-0 right-0 w-20 h-20 bg-gradient-to-br ${region.color} opacity-20 blur-2xl group-hover:opacity-40 transition-opacity duration-500`} />
                    ...
                    <div className="relative z-10 p-6 h-full flex flex-col justify-center">
                      ...
                      <div className="text-center">
                        <div className="text-5xl mb-3 group-hover:scale-125 group-hover:rotate-12 transition-all duration-500 drop-shadow-2xl">
                          {region.emoji}
                        </div>
                  ...
                  {/* 外围光环 */}
                  <div className={`absolute inset-0 rounded-3xl bg-gradient-to-r ${region.color} opacity-0 group-hover:opacity-30 blur-xl -z-10 transition-opacity duration-700`} />
                </Link>
              ))}
            </div>
          </div>
