...
// 基于真实爬取数据的北关东花见景点（三县合并）
const kitaKantoHanamiEvents = [
  {
...
const regionConfig = {
  ...
  displayName: '北关东',
  emoji: '🏔️',
  description: '茨城・栃木・群馬の美しい花見景点',
  navigationLinks: {
    prev: { name: '神奈川县', url: '/kanagawa/hanami', emoji: '⛩️' },
    next: { name: '甲信越', url: '/koshinetsu/hanami', emoji: '⛰️' },
    current: { name: '北关东', url: '/kitakanto' },
  },
};

export default function KitaKantoHanamiPage() {
  return (
    <HanamiPageTemplate
      region={regionConfig}
      events={kitaKantoHanamiEvents}
      regionKey="kitakanto"
      activityKey="hanami"
      pageTitle="北关东花见会活动列表"
      pageDescription="🏔️北关东花见会🌸 体验茨城・栃木・群馬三县最美樱花盛开，感受关东北部山间的春日浪漫。"
    />
  );
}
