...
export class MatsuriDataService {
  ...
  private async ensureDataDirectory() {
    try {
      ...
      console.error('Error creating data directory:', error instanceof Error ? error.message : String(error));
    }
  }
  ...
  // 保存祭典数据
  async saveMatsuriData(prefecture: string, data: MatsuriEvent[]): Promise<void> {
    try {
      ...
      console.error('Error saving matsuri data:', error instanceof Error ? error.message : String(error));
      throw error;
    }
  }
  ...
  async loadMatsuriData(prefecture: string = 'tokyo'): Promise<MatsuriEvent[]> {
    try {
      ...
      console.log(`No existing data found for ${prefecture}, returning empty array`);
      return [];
    }
  }
  ...
  async updateMatsuriData(prefecture: string = 'tokyo'): Promise<CrawlResult> {
    ...
    try {
      ...
      console.error('Error updating matsuri data:', error instanceof Error ? error.message : String(error));
      return {
        success: false,
        data: [],
        errors: [`Update error: ${error instanceof Error ? error.message : String(error)}`],
  ...
  // 合并祭典数据（去重和更新）
  private mergeMatsuriData(existing: MatsuriEvent[], newData: MatsuriEvent[]): MatsuriEvent[] {
    const merged = [...existing];
    
    newData.forEach(newEvent => {
      const existingIndex = merged.findIndex(
        existing => existing.title === newEvent.title && existing.date === newEvent.date
      );
  ...
  async getMatsuriStats(prefecture: string = 'tokyo'): Promise<{
    ...
    const stats = {
      ...
      lastUpdated: data.length > 0 ? data[0].lastUpdated : new Date().toISOString()
    };
  ...
  // 更新点赞数
  async updateLikes(prefecture: string, eventId: string, likes: number): Promise<void> {
    const data = await this.loadMatsuriData(prefecture);
    ...
    try {
      ...
      data.forEach((event, index) => {
        ...
        if (eventDate < oneYearAgo || eventDate > twoYearsLater) {
          warnings.push(`Event "${event.title}" has unusual date: ${event.date}`);
        }
