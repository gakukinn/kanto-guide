...
import { MatsuriEvent, MatsuriEventSchema, CrawlResult, DataSource } from '@/types/matsuri';
import { z } from 'zod';
...
export class MatsuriCrawler {
    ...
    this.crawler = new PlaywrightCrawler({
      ...
      requestHandler: async ({ page, request, log }) => {
        try {
          ...
          this.errors.push(`Error processing ${request.url}: ${error instanceof Error ? error.message : String(error)}`);
        }
      },
      failedRequestHandler: async ({ request, error }) => {
        this.errors.push(`Failed to process ${request.url}: ${error instanceof Error ? error.message : String(error)}`);
      },
    });
  }
  ...
  private async scrapeOmaturiLink(page: any) {
    const festivals = await page.evaluate(() => {
      ...
      // 查找祭典列表项
      const festivalElements = document.querySelectorAll('.festival-item, .event-item, .matsuri-item');
      
      festivalElements.forEach((element: Element) => {
        try {
          const title = element.querySelector('.title, .name, h3, h2')?.textContent?.trim();
          const date = element.querySelector('.date, .time')?.textContent?.trim();
          const location = element.querySelector('.location, .place')?.textContent?.trim();
    ...
    festivals.forEach((festival: any, index: number) => {
      try {
        const processedFestival = this.processFestivalData(festival, 'omaturilink.com');
        this.results.push(processedFestival);
      } catch (error) {
        this.errors.push(`Error processing festival ${index}: ${error instanceof Error ? error.message : String(error)}`);
      }
    });
  }
  ...
  private async scrapeMatcha(page: any) {
    const festivals = await page.evaluate(() => {
      ...
      // MATCHA 特定的选择器
      const festivalElements = document.querySelectorAll('article, .article-item, .event-card');
      
      festivalElements.forEach((element: Element) => {
        try {
          const title = element.querySelector('h1, h2, h3, .title')?.textContent?.trim();
          const content = element.querySelector('.content, .description, p')?.textContent?.trim();
    ...
    festivals.forEach((festival: any, index: number) => {
      try {
        const processedFestival = this.processFestivalData(festival, 'matcha-jp.com');
        this.results.push(processedFestival);
      } catch (error) {
        this.errors.push(`Error processing MATCHA festival ${index}: ${error instanceof Error ? error.message : String(error)}`);
      }
    });
  }
  ...
  private async scrapeRealJapanOn(page: any) {
    const festivals = await page.evaluate(() => {
      ...
      const festivalElements = document.querySelectorAll('.festival-card, .event-info, .matsuri-info');
      
      festivalElements.forEach((element: Element) => {
        try {
          const title = element.querySelector('.festival-name, .event-title, h3')?.textContent?.trim();
          const date = element.querySelector('.date, .schedule')?.textContent?.trim();
          const location = element.querySelector('.venue, .location')?.textContent?.trim();
          const description = element.querySelector('.description, .summary')?.textContent?.trim();
    ...
    festivals.forEach((festival: any, index: number) => {
      try {
        const processedFestival = this.processFestivalData(festival, 'on-japan.jp');
        this.results.push(processedFestival);
      } catch (error) {
        this.errors.push(`Error processing Real Japan festival ${index}: ${error instanceof Error ? error.message : String(error)}`);
      }
    });
  }

  // 处理和标准化祭典数据
  private processFestivalData(rawData: any, sourceDomain: string): MatsuriEvent {
    // 生成唯一ID
    ...
    const highlights = this.extractHighlights(rawData.title, rawData.description);
  ...
  // 推断祭典规模
  private inferCategory(title: string, description: string): '大型' | '中型' | '小型' {
    const text = `${title} ${description}`.toLowerCase();
    
    if (text.includes('大祭') || text.includes('花火大会') || text.includes('大規模')) {
      return '大型';
    } else if (text.includes('地域') || text.includes('小規模') || text.includes('地元')) {
      return '小型';
    }
  ...
  private extractHighlights(title: string, description: string): string[] {
    ...
    const keywords = [
      ...
      { pattern: /神輿|みこし/, highlight: '神轿游行' },
      { pattern: /祭り|祭典/, highlight: '传统祭典' },
      { pattern: /踊り|舞踊/, highlight: '传统舞蹈' },
  ...
  private estimateVisitors(category: '大型' | '中型' | '小型'): string {
    switch (category) {
      case '大型': return '10万人以上';
      case '中型': return '1-10万人';
      case '小型': return '1万人以下';
      default: return '1-10万人';
    }
  }
  ...
  private generateWebsiteUrl(title: string): string {
    ...
    return `https://www.google.com/search?q=${searchQuery}+公式サイト`;
  }
  ...
  async crawl(urls: string[]): Promise<CrawlResult> {
    try {
      ...
      return {
        success: false,
        data: [],
        errors: [`Crawler failed: ${error instanceof Error ? error.message : String(error)}`],
        timestamp: new Date().toISOString(),
        source: 'matsuri-crawler'
      };
    }
