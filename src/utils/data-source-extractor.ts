...
export interface ActivityDataSource {
  ...
  dataIntegrity: {
    ...
    pathIdConsistency: boolean;
  };
}

export interface ExtractionReport {
  totalActivities: number;
  activitiesWithOfficialWebsite: number;
  activitiesWithWalkerPlus: number;
  activitiesWithBothSources: number;
  activitiesWithNoSources: number;
...
export class DataSourceExtractor {
  private activities: ActivityDataSource[] = [];
  private regions = [
    'tokyo',
    'kanagawa',
    'chiba',
  ...
  private async extractFromRegion(region: string): Promise<void> {
    console.log(`\n🗂️  扫描区域: ${region}`);
  ...
  private async extractFromActivity(
    region: string,
    activityDir: string
  ): Promise<void> {
    const pageFile = resolve(
      `src/app/${region}/hanabi/${activityDir}/page.tsx`
    );
    ...
    try {
      ...
      for (const pattern of patterns) {
        ...
        if (dataImportMatch) break;
      }

      if (!dataImportMatch) {
        // 尝试查找页面内嵌数据
        const inlineDataFound = await this.extractInlineData(
          region,
          activityDir,
          pageContent
        );
      ...
      for (const key of dataKeys) {
        ...
        const activity = this.createActivityDataSource(
          region,
          activityDir,
          pageFile,
          fullDataPath,
  ...
  private createActivityDataSource(
    region: string,
    activityDir: string,
    pageFile: string,
    dataPath: string,
    ...
    // 检查路径与ID的一致性
    const pathIdConsistency =
      pathId === dataId || dataId.includes(pathId) || pathId.includes(dataId);
    ...
    return {
      id: dataId,
      name: data.name || '',
      nameJapanese: (data as any).nameJapanese,
      region,
      filePath: dataPath,
      pageFile,
  ...
  private async extractInlineData(
    region: string,
    activityDir: string,
    pageContent: string
  ): Promise<boolean> {
    try {
      // 查找页面中定义的数据对象
      const dataPattern =
        /const\s+\w*[dD]ata\s*:\s*HanabiData\s*=\s*\{([\s\S]*?)\};/;
      ...
      const activity: ActivityDataSource = {
        id,
        name,
        region,
  ...
  private logActivityExtraction(activity: ActivityDataSource): void {
    const sources = [];
    ...
    console.log(
      `    ${integrityIcon} ${activity.name} (${activity.id}) ${sourcesText}`
    );

    if (!activity.dataIntegrity.pathIdConsistency) {
      console.log(
        `      ⚠️  路径ID不一致: 目录=${activity.pageFile.split('/').pop()?.replace('/page.tsx', '')} vs 数据ID=${activity.id}`
      );
    }
  }
  ...
  private generateReport(): ExtractionReport {
    ...
    const activitiesWithOfficialWebsite = this.activities.filter(
      a => a.hasOfficialWebsite
    ).length;
    const activitiesWithWalkerPlus = this.activities.filter(
      a => a.hasWalkerPlusUrl
    ).length;
    const activitiesWithBothSources = this.activities.filter(
      a => a.hasOfficialWebsite && a.hasWalkerPlusUrl
    ).length;
    const activitiesWithNoSources = this.activities.filter(
      a => !a.hasOfficialWebsite && !a.hasWalkerPlusUrl
    ).length;

    const dataIntegrityIssues = this.activities.filter(
      a =>
        !a.dataIntegrity.hasValidId ||
        !a.dataIntegrity.hasValidName ||
        !a.dataIntegrity.hasTimeLocation ||
        !a.dataIntegrity.pathIdConsistency
    ).length;

    const regionBreakdown: { [region: string]: number } = {};
    this.regions.forEach(region => {
      regionBreakdown[region] = this.activities.filter(
        a => a.region === region
      ).length;
    });
  ...
  private printDetailedReport(report: ExtractionReport): void {
    console.log('\n📊 === 阶段1完成：数据源提取报告 ===');
    ...
    if (report.dataIntegrityIssues > 0) {
      ...
      report.activities.forEach(activity => {
        const issues = [];
        if (!activity.dataIntegrity.hasValidId) issues.push('缺少ID');
        if (!activity.dataIntegrity.hasValidName) issues.push('缺少名称');
        ...
        if (issues.length > 0) {
          console.log(
            `  ❌ ${activity.name || '未命名'} (${activity.region}): ${issues.join(', ')}`
          );
        }
      });
    }
    ...
    console.log('\n🎯 数据源优先级分析:');
    console.log(
      `🟢 高优先级 (有官网): ${report.activitiesWithOfficialWebsite} 个`
    );
    console.log(
      `🟡 中优先级 (仅WalkerPlus): ${report.activitiesWithWalkerPlus - report.activitiesWithBothSources} 个`
    );
    console.log(`🔴 低优先级 (无外部源): ${report.activitiesWithNoSources} 个`);
  }

  // 获取结果的公共方法
  public getResults(): ActivityDataSource[] {
    return this.activities;
  }
}

// 便捷执行函数
export async function extractDataSources(): Promise<ExtractionReport> {
