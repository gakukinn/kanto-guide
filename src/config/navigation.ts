...
export const JULY_REGIONS: NavigationRegion[] = [
  ...
  },
  {
    name: '北关东花火',
    slug: 'kitakanto',
    href: '/july/hanabi/kitakanto',
    emoji: '🏔️',
  },
  {
    name: '甲信越花火',
    slug: 'koshinetsu',
    href: '/july/hanabi/koshinetsu',
    emoji: '🗻',
  },
];
...
export const MATSURI_REGIONS: NavigationRegion[] = [
  ...
  { name: '神奈川', slug: 'kanagawa', href: '/kanagawa/matsuri', emoji: '⛩️' },
  {
    name: '北关东',
    slug: 'kitakanto',
    href: '/kitakanto/matsuri',
    emoji: '🏔️',
  },
  {
    name: '甲信越',
    slug: 'koshinetsu',
    href: '/koshinetsu/matsuri',
    emoji: '🗻',
  },
];
...
export function getRegionNavigation(currentSlug: string) {
  ...
  // 循环导航：前一个和后一个
  const prevIndex =
    (currentIndex - 1 + JULY_REGIONS.length) % JULY_REGIONS.length;
...
export function getMatsuriRegionNavigation(currentSlug: string) {
  const currentIndex = MATSURI_REGIONS.findIndex(
    region => region.slug === currentSlug
  );
  ...
  // 循环导航：前一个和后一个
  const prevIndex =
    (currentIndex - 1 + MATSURI_REGIONS.length) % MATSURI_REGIONS.length;
...
export function getHanabiRegionNavigation(currentSlug: string) {
  const currentIndex = HANABI_REGIONS.findIndex(
    region => region.slug === currentSlug
  );
  ...
  // 循环导航：前一个和后一个
  const prevIndex =
    (currentIndex - 1 + HANABI_REGIONS.length) % HANABI_REGIONS.length;
