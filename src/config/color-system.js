...
export function validateColorConsistency() {
  ...
  Object.entries(REGION_COLORS).forEach(([key, config]) => {
    // 检查必要属性
    const requiredFields = ['name', 'emoji', 'regionColor', 'activityColor', 'pageBackground', 'textColor', 'borderColor'];
    ...
    if (regionStartMatch && activityEndMatch) {
      ...
      if (config.pageBackground !== expectedBg) {
        errors.push(`${key} 背景色组合错误: 期望 ${expectedBg}, 实际 ${config.pageBackground}`);
      }
    }
  });
