...
interface IlluminationEvent {
  ...
  description: string;
  // 灯光秀特有字段 - 双字段格式支持
  lightCount?: number | string; // 原始格式（如"50万盏"）或数字
  lightCountNum?: number | null; // 标准化数字（如500000），null表示未公布
  expectedVisitors?: string; // 原始格式（如"12万人"）
...
export default function IlluminationPageTemplate({
  ...
  const validateAndFixEvents = (events: IlluminationEvent[]): IlluminationEvent[] => {
    console.log('🔍 开始灯光秀数据验证和修复...');
    ...
    const fixedEvents = events.map((event, index) => {
      ...
      if (!fixed.title && !fixed.name) {
        console.warn(`⚠️ 灯光秀事件 ${index} 缺少标题字段`);
        fixed.title = `未命名灯光秀 ${index + 1}`;
      }

      // 2. 修复日期字段不匹配问题
      if (!fixed.date && !fixed.dates) {
        console.warn(`⚠️ 灯光秀事件 ${index} 缺少日期字段`);
        fixed.date = '日期待定';
      }
      ...
      if (
        ...
        console.warn(`⚠️ 灯光秀事件 ${index} 红心数异常: ${fixed.likes}`);
    ...
    console.log(`✅ 灯光秀数据验证完成，处理了 ${fixedEvents.length} 个事件`);
    return fixedEvents;
  };
  ...
  const getRegionNavigation = () => {
    ...
    const regionCycle = [
      ...
      { key: 'saitama', name: '埼玉县', emoji: '🌸', url: '/saitama/illumination' },
  ...
  const addIconToFeature = (feature: string): string => {
    ...
    const iconMappings = [
      // 灯光秀相关
      { keywords: ['万盏', '盏', '灯光', '打上'], icon: '💡' },
      { keywords: ['スターマイン', 'ナイアガラ', '仕掛灯光'], icon: '💡' },
      ...
      <nav className="pb-2 pt-4">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-center space-x-2 text-gray-600">
            ...
            <span className="text-gray-400">›</span>
            <span className="font-medium text-blue-600">💡 灯光秀</span>
          </div>
        </div>
      </nav>
      ...
      <section className="pb-12 pt-12 text-center">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="mx-auto max-w-7xl text-lg leading-relaxed text-gray-700 md:text-xl">
            {pageDescription ||
              `体验${region.displayName}最精彩的灯光秀，感受${region.description}`}
          </p>
        </div>
      </section>
...
export const fetchAndValidateIlluminationData = async (
  ...
  try {
    ...
    const validatedEvents = events.map((event: any, index: number) => ({
      ...
      name: event.name || event.title || `灯光秀 ${index + 1}`,
      title: event.title || event.name || `灯光秀 ${index + 1}`,
      ...
      lightCount: event.lightCount || null,
      expectedVisitors: event.expectedVisitors || null,
      venue: event.venue || event.location || '会场待定',
    }));

    console.log(`✅ 灯光秀数据获取成功，共 ${validatedEvents.length} 个事件`);
    return validatedEvents;
  } catch (error) {
    console.error('灯光秀数据获取失败:', error);
    return [];
  }
};
