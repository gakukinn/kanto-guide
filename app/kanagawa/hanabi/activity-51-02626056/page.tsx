/**
 * 第51回 金沢祭典 花火大会 详情页面
 * 自动生成于 2025-06-29T05:52:46.453Z
 * 使用模板: WalkerPlusHanabiTemplate
 */
import WalkerPlusHanabiTemplate from '../../../../src/components/WalkerPlusHanabiTemplate';

const pageData = {
  "name": "第51届金泽节烟花大会",
  "fireworksCount": "約3500発",
  "fireworksTime": "60分",
  "expectedVisitors": "約25万5000人",
  "date": "2025年8月30日(土)",
  "time": "19：00 - 20：00（预定）",
  "venue": "海洋公园",
  "access": "[火车]从JR新杉田站或京滨急行金泽八景站乘横滨海滨线，在海上公园南口站或海上公园芝口站下车。从京滨急行金泽文库站步行20分钟",
  "weatherInfo": "恶劣天气停止，不拖延。",
  "parking": "* 请使用公共交通工具。",
  "price": "无",
  "contact": "050-3159-6791",
  "foodStalls": "有",
  "notes": "从前一天起就被禁止了。在包括沙滩在内的海洋公园内禁止吸烟。宠物不允许入境。垃圾带回家。活动当天非常拥挤，因此不能乘坐汽车、摩托车、自行车来。周围道路在18：30~21：00有交通限制。",
  "website": "https://www.city.yokohama.lg.jp/kanazawa/kurashi/kyodo_manabi/kyodo_shien/chiiki/kanazawamaturi/maturi2025/maturi.html",
  "googleMap": "https://www.google.com/maps/embed/v1/place?key=AIzaSyDbSCIe9u9tyzmG1OdAae8eRLPGmK5sevc&zoom=16&q=35.340969,139.635978&center=35.340969,139.635978",
  "id": "1751102626058",
  "region": "kanagawa",
  "activityType": "hanabi",
  "description": "烟花大会在横滨市金泽区海之公园举行。大约3500发烟花将在60分钟内燃放。从被八景岛、海洋公园和野岛环绕的海面上，五颜六色的烟花在横滨的夜空中燃放。各种各样的连续花火系列为活动增添了华丽的色彩，而一年一度的大型连续花火则为决赛增添了色彩。",
  "highlights": "入场是免费的，你可以坐在沙滩上或公园的草坪上欣赏烟花。",
  "themeColor": "red",
  "status": "scheduled",
  "media": [
    {
      "type": "image" as const,
      "url": "https://assets.st-note.com/production/uploads/images/197217762/rectangle_large_type_2_dc7306e749f35b7e8beae385e92e0d63.png?width=2000&height=2000&fit=bounds&quality=85",
      "title": "第51回 金沢祭典 花火大会图片1",
      "alt": "第51回 金沢祭典 花火大会图片1",
      "caption": ""
    }
  ],
  "detailLink": "/kanagawa/hanabi/activity-51-02626056",
  "createdAt": "2025-06-29T05:52:46.452Z",
  "source": "walkerplus-generator"
} as const;

export default function DetailPage() {
  return (
    <WalkerPlusHanabiTemplate
      data={pageData}
      regionKey="kanagawa"
      activityKey="hanabi"
    />
  );
}

export const metadata = {
  title: '第51回 金沢祭典 花火大会 - 日本活动指南',
  description: '横浜市金沢区海の公園で举办花火大会。60分間にわたって約3500発の花火が被燃放。八景島、海の公園、野島に囲まれた海上から、横浜の夜空に色とりどりの花火が打燃放がる。多様な连发烟花の連続が大会を華やかに彩り、毎年恒例の特大连发烟花がフィナーレを飾る。',
};
