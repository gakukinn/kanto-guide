...
const autoDetectActivityType = (text: string): ActivityType | null => {
  const keywords = {
    matsuri: ['祭り', '祭典', 'まつり', 'matsuri', '祭', '祭事', '神社', '神輿', 'みこし', '盆踊り', '縁日']
  };
  
  const lowerText = text.toLowerCase();
  if (keywords.matsuri.some(word => lowerText.includes(word.toLowerCase()))) {
    return 'matsuri';
  }
  return null;
};
...
export default function WalkerPlusMatsuriGeneratorPage() {
  // 基本状态
  ...
  const handleTextRecognition = async () => {
    ...
    try {
      // 🏮 使用新的祭典专用文本解析器
      const response = await fetch('/api/matsuri-text-parser', {
        method: 'POST',
      ...
      if (data.success) {
        // 🏮 处理祭典解析的数据，保持兼容性
        const textResultWithMatsuriFields = {
          ...data.data,
          // 🏮 使用祭典字段数组
          walkerFields: data.matsuriFields,
          // 🏮 保持字段名兼容性
          name: data.data.eventName || data.data.standardData?.name,
          reservationSystem: data.data.reservationRequired || '',
          viewingPoints: data.data.recommendedViewpoint || ''
        };
        setTextResult(textResultWithMatsuriFields);
        
        // 自动识别地区
        const detectedRegion = autoDetectRegion(data.data.spotAddress || data.data.venue || data.data.venueAccess || '');
  ...
  const handlePageGeneration = async () => {
    ...
    if (currentData && textResult && !urlResult) {
      Object.assign(currentData, {
        ...
        reservationSystem: textResult.reservationSystem || currentData.reservationSystem,
        viewingPoints: textResult.viewingPoints || currentData.viewingPoints
      });
    }
    ...
    try {
      const response = await fetch('/api/walkerplus-matsuri-generator', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      <div className="max-w-6xl mx-auto p-6">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            🏮 WalkerPlus 祭典四层页面生成器
          </h1>
          <p className="text-gray-600">
            基于 WalkerPlus 官方数据，自动生成祭典活动四层页面结构。支持文本识别和URL爬取两种方式。
          </p>
          ...
          <div className="mt-4 p-3 bg-orange-50 border border-orange-200 rounded-lg">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="text-orange-600">
                生成的数据将自动出现在 
                <a href="/admin/third-layer-generator" target="_blank" className="underline hover:text-orange-800">
                  三层页面生成器
                </a>
              </span>
            </div>
        ...
        <div className="mb-8">
          <div className="bg-white rounded-lg border-2 border-orange-200 shadow-md">
            <div className="p-4 border-b bg-gradient-to-r from-orange-50 to-orange-100">
              ...
              <p className="text-sm text-orange-700 mt-1">粘贴 WalkerPlus 格式文本，自动识别祭典活动信息</p>
            </div>
...
...`}
                rows={8}
                className="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-orange-500 focus:border-orange-500 font-mono text-sm resize-none"
              />
              ...
              <div className="flex gap-2 mt-3">
                <button 
                  ...
                  className="flex-1 px-4 py-2 bg-orange-600 text-white rounded-md hover:bg-orange-700 disabled:opacity-50 disabled:cursor-not-allowed font-medium text-sm transition-colors"
                >
                ...
                <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-md">
                  ...
                  {(textResult as any).walkerFields ? (
                    <div className="space-y-1">
                      <div className="text-xs font-medium text-gray-600 mb-2">📊 祭典信息：</div>
        ...
        <div className="mb-8">
          <div className="bg-white rounded-lg border-2 border-red-200 shadow-md">
            <div className="p-4 border-b bg-gradient-to-r from-red-50 to-red-100">
              ...
              <p className="text-sm text-red-700 mt-1">输入 WalkerPlus URL，自动爬取祭典活动信息</p>
            </div>
            ...
            <div className="p-4">
              ...
              <input
                ...
                placeholder="https://matsuri.walkerplus.com/..."
              ...
              <div className="flex gap-2 mt-3">
                <button 
                  ...
                  disabled={urlLoading}
                  className="flex-1 px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed font-medium text-sm transition-colors"
                >
                  {urlLoading ? '爬取中...' : '开始爬取'}
                </button>
                <button
          ...
          <div className="mb-8">
            <div className="bg-white rounded-lg border-2 border-orange-200 shadow-md">
              <div className="p-4 border-b bg-gradient-to-r from-orange-50 to-orange-100">
                ...
                <p className="text-sm text-orange-700 mt-1">选择活动类型和地区，系统已自动识别</p>
              </div>
              ...
              <div className="p-4">
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <div className="w-full p-3 border border-gray-300 rounded-lg bg-orange-50">
                      <div className="flex items-center gap-2">
                        <span className="text-2xl">🏮</span>
                        <span className="font-medium text-orange-800">传统祭典</span>
                        <span className="text-sm text-orange-600">(WP专用)</span>
                      </div>
                    </div>
                    <p className="mt-1 text-xs text-orange-600">✨ WalkerPlus祭典生成器专门用于传统祭典活动</p>
                  </div>
          ...
          <div className="mb-8">
            <div className="bg-white rounded-lg border-2 border-red-200 shadow-md">
              <div className="p-4 border-b bg-gradient-to-r from-red-50 to-red-100">
                ...
                <p className="text-sm text-red-700 mt-1">为祭典活动添加图片，支持智能搜索</p>
              </div>
          ...
          <div className="mb-8">
            <div className="bg-white rounded-lg border-2 border-orange-200 shadow-md">
              <div className="p-4 border-b bg-gradient-to-r from-orange-50 to-orange-100">
                ...
                <p className="text-sm text-orange-700 mt-1">生成祭典四层页面和JSON文件</p>
              </div>
              
              <div className="p-4">
                {/* 覆盖选项 - 默认关闭，让用户选择 */}
                <div className={`mb-4 p-4 border rounded-lg ${forceOverwrite ? 'bg-red-50 border-red-200' : 'bg-orange-50 border-orange-200'}`}>
                  <label className="flex items-center space-x-3 cursor-pointer">
                    ...
                    <div className="flex-1">
                      <div className={`text-sm font-semibold ${forceOverwrite ? 'text-red-800' : 'text-orange-800'}`}>
                        {forceOverwrite ? '🔄 智能覆盖模式' : '🆕 安全模式（推荐）'}
                      </div>
                      <div className={`text-xs ${forceOverwrite ? 'text-red-700' : 'text-orange-700'}`}>
                        {forceOverwrite 
                          ? '检测到重复活动时自动覆盖现有页面和数据' 
                ...
                <button
                  ...
                  className="w-full px-6 py-3 bg-gradient-to-r from-orange-600 to-red-600 text-white rounded-lg hover:from-orange-700 hover:to-red-700 disabled:opacity-50 disabled:cursor-not-allowed font-medium text-lg transition-all transform hover:scale-105"
                >
                  {generating ? '生成中...' : `🏮 生成祭典四层页面 ${forceOverwrite ? '(自动覆盖)' : '(安全模式)'}`}
                </button>

                {/* 生成进度 */}
                {generationProgress && (
                  <div className="mt-4 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                    <div className="text-sm text-orange-800">{generationProgress}</div>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
              <div className="p-6 border-b bg-gradient-to-r from-orange-50 to-red-50">
                ...
                <p className="text-sm text-orange-700 mt-1">
                  系统检测到 {conflictData.similarActivities?.length || 0} 个高度相似的祭典活动，请仔细对比后选择处理方式
                </p>
              </div>
              ...
              <div className="p-6">
                ...
                <div className="mb-6 p-4 bg-orange-50 border border-orange-200 rounded-lg">
                  <h4 className="font-semibold text-orange-800 mb-3">🆕 当前要生成的祭典活动</h4>
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div><span className="font-medium">预约制：</span>{conflictData.currentActivity?.reservationSystem || '未知'}</div>
                  </div>
                </div>
                    ...
                    {conflictData.similarActivities?.map((item: any, index: number) => (
                      <div key={index} className="p-4 border border-gray-200 rounded-lg bg-gray-50">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          ...
                          <div><span className="font-medium">预约制：</span>{item.activity?.reservationSystem || '未知'}</div>
                          <div><span className="font-medium">观赏点：</span>{item.activity?.viewingPoints || '未知'}</div>
                        </div>
