...
async function testSimpleAPI() {
  ...
  console.log('🚀 测试简化版API (HTTP + Cheerio)');
  console.log('='.repeat(60));
  
  for (let i = 0; i < urls.length; i++) {
    const url = urls[i];
    const encodedUrl = encodeURIComponent(url);
    const apiUrl = `http://localhost:3000/api/auto-import-simple/?url=${encodedUrl}`;
          ...
          res.on('end', () => {
            try {
              ...
              if (result.success) {
                ...
                // 显示详细数据
                console.log('\n📝 详细数据:');
                console.log(`   名称: "${result.parsed.name}"`);
                console.log(`   地址: "${result.parsed.address}"`);
                console.log(`   期间: "${result.parsed.period}"`);
                console.log(`   场所: "${result.parsed.venue}"`);
                console.log(`   交通: "${result.parsed.access}"`);
                console.log(`   主办: "${result.parsed.organizer}"`);
                console.log(`   价格: "${result.parsed.price}"`);
                console.log(`   联系: "${result.parsed.contact}"`);
                console.log(`   网站: "${result.parsed.website}"`);
              } else {
