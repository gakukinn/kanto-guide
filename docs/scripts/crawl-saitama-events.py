...
"""
埼玉县活动十项信息精确爬取工具
专门爬取 jalan.net 埼玉县活动页面的前10个活动详情
"""
...
class SaitamaEventsCrawler:
    def __init__(self):
        self.base_url = "https://www.jalan.net/event/110000/?screenId=OUW1702"
        self.target_fields = [
            "名称", "所在地", "開催期間", "開催場所", 
            "交通アクセス", "主催", "料金", "問合せ先", 
            "ホームページ", "谷歌网站"
    ...
    def extract_accurate_fields(self, markdown_content, event_link):
        ...
        for i, line in enumerate(lines):
            ...
            if " | " in line and in_basic_info_table:
                ...
                if len(parts) == 2:
                    ...
                    if "所在地" in key:
                        # 提取地址中的都道府县
                        prefecture_match = re.search(r'(埼玉県|東京都|大阪府|京都府|[^市区町村]+県)', value)
                        if prefecture_match:
        ...
        if event_info["ホームページ"]:
            ...
            # 使用活动名称搜索
            search_query = f"{event_info['名称']} 埼玉県"
    ...
    async def crawl_top_10_accurate_events(self):
        ...
        with open("saitama_events_accurate_ten_fields.json", 'w', encoding='utf-8') as f:
            json.dump(final_result, f, ensure_ascii=False, indent=2)
        
        print(f"\n🎉 精确爬取完成！")
        print(f"📊 成功爬取 {len(all_events_info)} 个活动的十项准确信息")
        print(f"📁 结果文件: saitama_events_accurate_ten_fields.json")
        
        # 打印详细汇总表格
        print("\n📋 埼玉县活动爬取结果详细汇总:")
        print("=" * 150)
        print(f"{'序号':<4} {'活动名称':<30} {'开催期间':<20} {'开催场所':<25} {'主催':<20} {'问合せ先':<15}")
        print("=" * 150)
        for i, event in enumerate(all_events_info, 1):
            name = event['名称'][:28] + '...' if len(event['名称']) > 30 else event['名称']
            ...
            print(f"{i:<4} {name:<30} {period:<20} {venue:<25} {organizer:<20} {contact:<15}")
        print("=" * 150)
        
        return all_events_info

async def main():
    crawler = SaitamaEventsCrawler()
