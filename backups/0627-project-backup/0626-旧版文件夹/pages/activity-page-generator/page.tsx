...
const autoDetectRegion = (text: string): RegionType | null => {
  const regionKeywords = {
    tokyo: ['東京', '东京', '新宿', '渋谷', '涉谷', '池袋', '銀座', '银座', '浅草', '上野', '秋葉原', '秋叶原', '六本木', '原宿', '品川', '新橋', '新桥'],
    saitama: ['埼玉', '大宮', '大宫', '浦和', 'さいたま', '川口', '所沢', '越谷', '春日部', '草加', '朝霞'],
    chiba: ['千葉', '千叶', '船橋', '船桥', '柏', '市川', '松戸', '木更津', '成田', '佐倉', '習志野', '习志野'],
    kanagawa: ['神奈川', '横浜', '横滨', '川崎', '相模原', '藤沢', '茅ヶ崎', '茅崎', '平塚', '鎌倉', '镰仓', '厚木'],
    kitakanto: ['群馬', '群马', '栃木', '茨城', '前橋', '前桥', '高崎', '宇都宮', '宇都宫', '水戸', '水户', 'つくば', '筑波'],
    koshinetsu: ['山梨', '長野', '长野', '新潟', '甲府', '松本', '長岡', '长冈', '上越', '富士河口湖', '諏訪', '诹访']
  };
            ...
            <div className="mb-8">
              <div className="bg-white rounded-lg border-2 border-blue-300 p-6 shadow-lg">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      🎭 选择活动类型 <span className="text-amber-600">(手动选择)</span>
                    </label>
                    <select
                      value={activityType}
                      onChange={(e) => {
                    ...
                    >
                      {Object.entries(ACTIVITY_CONFIGS).map(([key, config]) => (
                        <option key={key} value={key}>
                          {config.emoji} {config.name} - 使用 {config.template}
                        </option>
                      ))}
                    </select>
