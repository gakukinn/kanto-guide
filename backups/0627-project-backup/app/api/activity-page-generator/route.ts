...
// 🆕 JSON文件生成函数
async function generateJSONFiles(activityType: keyof typeof ACTIVITY_CONFIGS, data: any, region: string, uploadedImages: string[] = []) {
  ...
  try {
    ...
    const regionSummaryData = {
      ...
      detailLink: `/${region}/${activityType}/activity-${data.id.split('-').pop().slice(-8)}`,
      likes: 0,
      themeColor: 'red'
    };
        ...
        for (const file of jsonFiles) {
          try {
            ...
            if (existingData.region === region && existingData.activityType === activityType) {
              ...
              // 判断条件：名称相似度 >= 80% 或者 (名称相似度 >= 60% 且 (日期相似 或 地址相似)) 或者 (名称相似度 >= 20% 且 日期相似 且 地址相似)
              const overallSimilarity = nameSimilarity >= 0.8 ? nameSimilarity : 
                                      (nameSimilarity >= 0.6 && (dateSimilar || addressSimilar)) ? nameSimilarity :
                                      (nameSimilarity >= 0.2 && dateSimilar && addressSimilar) ? nameSimilarity : 0;
