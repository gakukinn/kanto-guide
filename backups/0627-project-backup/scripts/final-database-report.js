...
async function generateReport() {
  try {
    ...
    const matsuriInOtherTables = await runQuery(`
      SELECT 'hanabi_events' as table_name, name FROM hanabi_events WHERE name LIKE '%まつり%'
      UNION ALL
      SELECT 'hanami_events' as table_name, name FROM hanami_events WHERE name LIKE '%まつり%'
      UNION ALL
      SELECT 'momiji_events' as table_name, name FROM momiji_events WHERE name LIKE '%まつり%'
      UNION ALL
      SELECT 'culture_events' as table_name, name FROM culture_events WHERE name LIKE '%まつり%';
    `);
    
    if (matsuriInOtherTables && matsuriInOtherTables.trim()) {
      const misclassified = matsuriInOtherTables.split('\n').filter(m => m.trim());
      console.log(`⚠️ 发现 ${misclassified.length} 个带"まつり"的活动可能被错误分类:`);
      ...
      console.log('💡 建议: 所有带"まつり"的活动应该归类为传统祭典');
    } else {
      console.log('✅ 所有带"まつり"的活动都正确归类为传统祭典');
    }
