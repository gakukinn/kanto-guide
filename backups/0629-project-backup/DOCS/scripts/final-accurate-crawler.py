...
class AccurateEventsCrawler:
        ...
        for i, line in enumerate(lines):
            ...
            if " | " in line and in_basic_info_table:
                ...
                if len(parts) == 2:
                    ...
                    if "所在地" in key:
                        # 提取地址中的都道府县
                        prefecture_match = re.search(r'(東京都|大阪府|京都府|[^市区町村]+県)', value)
                        if prefecture_match:
    ...
    async def crawl_top_10_accurate_events(self):
        ...
        with open("tokyo_events_accurate_ten_fields.json", 'w', encoding='utf-8') as f:
            json.dump(final_result, f, ensure_ascii=False, indent=2)
        
        print(f"\n🎉 精确爬取完成！")
        print(f"📊 成功爬取 {len(all_events_info)} 个活动的十项准确信息")
        print(f"📁 结果文件: tokyo_events_accurate_ten_fields.json")
