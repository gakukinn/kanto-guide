...
"""
千叶县活动十项信息精确爬取工具
专门爬取 jalan.net 千叶县活动页面的前10个活动详情
"""
...
class ChibaEventsCrawler:
    def __init__(self):
        self.base_url = "https://www.jalan.net/event/120000/?screenId=OUW1702"
        self.target_fields = [
            "名称", "所在地", "開催期間", "開催場所", 
            "交通アクセス", "主催", "料金", "問合せ先", 
            "ホームページ", "谷歌网站"
    ...
    def extract_accurate_fields(self, markdown_content, event_link):
        ...
        for i, line in enumerate(lines):
            ...
            if " | " in line and in_basic_info_table:
                ...
                if len(parts) == 2:
                    ...
                    if "所在地" in key:
                        # 提取地址中的都道府县
                        prefecture_match = re.search(r'(千葉県|東京都|大阪府|京都府|[^市区町村]+県)', value)
                        if prefecture_match:
        ...
        if event_info["ホームページ"]:
            ...
            # 使用活动名称搜索
            search_query = f"{event_info['名称']} 千葉県"
    ...
    async def crawl_top_10_accurate_events(self):
        ...
        with open("chiba_events_accurate_ten_fields.json", 'w', encoding='utf-8') as f:
            json.dump(final_result, f, ensure_ascii=False, indent=2)
        
        print(f"\n🎉 精确爬取完成！")
        print(f"📊 成功爬取 {len(all_events_info)} 个活动的十项准确信息")
        print(f"📁 结果文件: chiba_events_accurate_ten_fields.json")
