...
const DATA_MAPPING = {
  ...
  'tokyo-racecourse': {
    ...
    officialName: '東京競馬場花火 2025 〜花火和聴了 J-POP BEST〜',
    expectedData: {
      date: '2025-07-02(水)',
  ...
  tachikawa: {
    ...
    officialName: '立川祭典 国営昭和記念公園花火大会',
    expectedData: {
      date: '2025-07-26(土)',
 ...
 * (实际应用中应该是真实的API调用)
 */
async function fetchWalkerPlusData(
  walkerPlusId: string
): Promise<WalkerPlusData | null> {
  // 模拟API延迟
  await new Promise(resolve => setTimeout(resolve, 100));

  // 返回模拟数据（基于我们从网站搜索得到的最新信息）
  const mapping = Object.values(DATA_MAPPING).find(
    m => m.walkerPlusId === walkerPlusId
  );
...
export async function updateHanabiDynamicData(
  ...
  try {
    ...
    if (!walkerData) {
      console.warn(
        `No data returned for Walker Plus ID: ${mapping.walkerPlusId}`
      );
      return currentData;
    }
    ...
    const updatedData: HanabiData = {
      ...
      // 如果日期有变化，更新日期
      date:
        walkerData.date !== mapping.expectedData.date
          ? walkerData.date
          : currentData.date,
      // 更新动态数据
      dynamicData,
 ...
 * 批量更新所有花火数据
 */
export async function batchUpdateAllHanabi(
  hanabiDataList: HanabiData[]
): Promise<HanabiData[]> {
  console.log(
    `🔄 Starting batch update for ${hanabiDataList.length} hanabi events...`
  );
  ...
  console.log(
    `✅ Batch update completed. Updated ${updatedList.length} events.`
  );
  return updatedList;
}
...
export function detectDataChanges(
  ...
  if (oldData.fireworksCount !== newData.fireworksCount) {
    changes.push(
      `花火数量变更: ${oldData.fireworksCount} → ${newData.fireworksCount}`
    );
  }
