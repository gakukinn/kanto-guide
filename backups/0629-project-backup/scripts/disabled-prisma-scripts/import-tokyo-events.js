...
// 爬取的活动数据文件路径
const DATA_FILE = path.join(__dirname, '..', 'tokyo_events_accurate_ten_fields.json');
...
function determineEventType(eventName) {
  if (eventName.includes('花火')) {
    return 'hanabi';
  } else if (eventName.includes('祭') || eventName.includes('まつり') || eventName.includes('祭典')) {
    return 'matsuri';
...
async function getTokyoRegion() {
  try {
    ...
    if (!tokyoRegion) {
      ...
      console.log('✅ 东京地区记录创建成功');
    } else {
      console.log('✅ 找到现有东京地区记录');
    }

    return tokyoRegion;
  } catch (error) {
    console.error('❌ 获取东京地区记录失败:', error);
    throw error;
  }
}
 ...
 * 主要录入函数
 */
async function importTokyoEvents() {
  console.log('🚀 开始录入东京活动信息到数据库...\n');
  ...
  try {
    ...
    // 2. 获取或创建东京地区记录
    const tokyoRegion = await getTokyoRegion();
    ...
    for (let i = 0; i < eventsData.events.length; i++) {
      ...
      try {
        const result = await insertEvent(eventData, eventType, tokyoRegion.id);
...
module.exports = { importTokyoEvents }; 