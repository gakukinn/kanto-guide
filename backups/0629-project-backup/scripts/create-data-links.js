...
function createDataLinks() {
  ...
  // 为每个地区创建数据文件链接
  Object.entries(dataMapping).forEach(([region, mappings]) => {
    const regionDir = path.join(srcDataDir, 'hanabi', region);
    ...
    Object.entries(mappings).forEach(([targetName, sourceName]) => {
      const sourceFile = path.join(srcDataDir, `${sourceName}.ts`);
      const targetFile = path.join(regionDir, `${targetName}.ts`);
      ...
      if (fs.existsSync(sourceFile)) {
        ...
        try {
          ...
          console.log(`📋 复制文件: ${region}/${targetName} -> ${sourceName}`);
        } catch (error) {
          console.log(`❌ 复制失败: ${region}/${targetName} - ${error.message}`);
        }
      } else {
        console.log(`❌ 源文件不存在: ${sourceName}.ts`);
      }
    });
  });
  
  console.log('🎉 数据文件映射完成！');
}
