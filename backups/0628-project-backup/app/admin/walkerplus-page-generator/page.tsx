...
const ACTIVITY_CONFIGS = {
  ...
  hanami: { name: '花见会', emoji: '🌸' },
  hanabi: { name: '花火会', emoji: '🎆' },
  momiji: { name: '红叶狩', emoji: '🍁' },
  illumination: { name: '灯光秀', emoji: '✨' },
  culture: { name: '文艺术', emoji: '🎭' }
};
...
const autoDetectActivityType = (text: string): ActivityType | null => {
  const keywords = {
    ...
    matsuri: ['祭', '祭り', 'まつり', '夏祭り', '例大祭'],
    hanami: ['花見', '花见', 'はなみ', '桜', '樱'],
    momiji: ['紅葉', '红叶', 'もみじ', '紅葉狩り'],
    illumination: ['イルミネーション', 'ライトアップ', '点灯', '灯光'],
    culture: ['文化', '芸術', '艺术', '展覧会', '美術館']
  };
...
export default function WalkerPlusPageGeneratorPage() {
  ...
  const handleTextRecognition = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const detectedType = autoDetectActivityType(data.data.name || '');
        ...
        if (detectedType) {
          setActivityType(detectedType);
          setAutoDetectedActivity(detectedType);
        }
      } else {
  ...
  const handleUrlScraping = async () => {
    ...
    try {
      ...
      const detectedType = autoDetectActivityType(data.name || '');
            ...
            <div className="bg-white rounded-lg border-2 border-purple-200 shadow-md">
              ...
              <div className="p-4">
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      🎭 活动类型 {autoDetectedActivity && <span className="text-green-600">(已自动识别)</span>}
                    </label>
                    <select
                      value={activityType}
                    ...
                    {conflictData.similarActivities?.map((item: any, index: number) => (
                      <div key={index} className="p-4 border border-gray-200 rounded-lg bg-gray-50">
                        ...
                        <div className="mt-3 p-2 bg-white rounded border text-xs">
                          <div className="font-medium text-gray-700 mb-1">相似度分析：</div>
                          <div className="grid grid-cols-3 gap-2">
                            <div>名称: {((item.similarityDetails?.name || 0) * 100).toFixed(1)}%</div>
                            <div>日期: {item.similarityDetails?.date ? '✅' : '❌'}</div>
                            <div>地址: {item.similarityDetails?.address ? '✅' : '❌'}</div>
                          </div>
                        </div>
