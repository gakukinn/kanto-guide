...
const generatePageFile = async (
  ...
  const standardData = {
    ...
    fireworksCount: data.fireworksCount || contactFields.fireworksCount || '',
    fireworksTime: data.fireworksTime || data.fireworksDuration || contactFields.fireworksTime || '',
    expectedVisitors: data.expectedVisitors || contactFields.expectedVisitors || '',
    date: data.date || data.eventPeriod || data.datetime || '',
    time: data.time || data.eventTime || '',
    venue: data.venue || '',
    access: data.access || data.venueAccess || '',
    weatherInfo: data.weatherInfo || data.weatherPolicy || contactFields.weatherInfo || '',
    parking: data.parking || contactFields.parking || '',
    price: data.price || data.paidSeats || '',
    contact: data.contact || data.contactInfo || '详见官网',
    foodStalls: data.foodStalls || contactFields.foodStalls || '',
    notes: data.notes || data.otherNotes || contactFields.notes || '',
...
const pageData = ${JSON.stringify(standardData, null, 2)};
...
const generateJsonFiles = async (
  ...
  try {
    // 1. 创建单个活动JSON文件 (data/activities/{id}.json) - Activity Generator标准
    const activitiesDir = path.join(process.cwd(), 'data', 'activities');
    await mkdir(activitiesDir, { recursive: true });
    
    const activityFilePath = path.join(activitiesDir, `${activityData.id}.json`);
    ...
    for (const file of jsonFiles) {
      try {
        ...
        if (existingData.region === region && existingData.activityType === activityType) {
          ...
          const dateSimilar = areDatesSimilar(activityData.date || '', existingData.date || '');
          const addressSimilar = areAddressesSimilar(activityData.venue || '', existingData.venue || '');
...
export async function POST(request: NextRequest) {
  try {
    ...
    const completeActivityData = {
      ...
      // 🔧 简化修复：直接使用URL爬取的description和highlights
      description: (() => {
        console.log('🔍 Description映射逻辑分析:');
        console.log('  data.description:', data.description || 'undefined');
        console.log('  data.description长度:', data.description ? data.description.length : 0);
        
        // 简化逻辑：如果description存在且长度超过活动名称，就使用它
        if (data.description && data.description.length > 50) {
          console.log('  ✅ 使用URL爬取的完整description');
          return data.description;
        }
        
        // 否则使用fallback
        const fallback = data.parsedDescription || data.name || data.eventName || '';
        console.log('  ⚠️ 使用fallback description:', fallback);
        return fallback;
      })(),
      highlights: (() => {
        console.log('🔍 Highlights映射逻辑分析:');
        console.log('  data.highlights:', data.highlights || 'undefined');
        console.log('  data.highlights长度:', data.highlights ? data.highlights.length : 0);
        
        // 简化逻辑：如果highlights存在且有实际内容，就使用它
        if (data.highlights && data.highlights.length > 10) {
          console.log('  ✅ 使用URL爬取的highlights');
          return data.highlights;
        }
        
        console.log('  ⚠️ 未设置highlights');
        return undefined;
      })()
    };
    
    console.log(`📊 最终数据映射: description=${completeActivityData.description ? '已设置' : '未设置'}, highlights=${completeActivityData.highlights ? '已设置' : '未设置'}`);
