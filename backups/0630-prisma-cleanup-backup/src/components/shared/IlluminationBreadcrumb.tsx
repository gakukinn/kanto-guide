...
export default function IlluminationBreadcrumb({
  ...
  const truncateTitle = (title: string, maxLength: number = 20) => {
    ...
    // 优先保留重要关键词
    const keywords = ['灯光', '彩灯', '点灯', '夜景', '公园', '神社', '寺', '展览', '节庆'];
    ...
    <nav className="pb-2 pt-4">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div className="flex items-center space-x-2 text-gray-600 overflow-hidden">
          <Link
            href={'/' as any}
            className="font-medium transition-colors hover:text-purple-600 whitespace-nowrap"
          >
            ⛩️ 首页
          </Link>
          ...
          <span className="text-gray-400 flex-shrink-0">›</span>
          <span 
            className="font-medium text-purple-600 truncate min-w-0" 
            title={illuminationName}
          >
            {truncateTitle(illuminationName)}
          </span>
        </div>
      </div>
    </nav>
