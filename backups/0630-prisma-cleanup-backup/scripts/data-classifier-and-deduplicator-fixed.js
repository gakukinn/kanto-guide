...
function classifyActivity(name, venue = '', organizer = '') {
  ...
  // 传统祭典关键词
  const matsuriKeywords = ['祭', '祭典', 'matsuri', '祇園', '神社', '盆踊り', '山車', '御輿', '神輿', '太鼓'];
  
  // 花见会关键词
  const hanamiKeywords = ['桜', 'さくら', 'sakura', '花見', '桜祭典', '桜祭り', 'cherry', 'blossom'];
    ...
    for (const table of tables) {
      try {
        const events = await runQuery(`
          SELECT id, name, venue, organizer, contact, website 
          FROM ${table.name}
        `);
        ...
        console.log(`⚠️ 查询表 ${table.name} 时出错:`, error.message);
      }
    }
