...
export class DataAccuracyValidator {
  ...
  validateEventData(
    ...
    if (
      eventData.visitors &&
      !['约', '万', '人', '非公开', '未定'].some(char =>
        eventData.visitors.includes(char)
      )
    ) {
      validation.checks.push({
        fieldName: 'visitors',
    ...
    if (
      eventData.fireworks &&
      !['发', '非公表', '未定', '约'].some(char =>
        eventData.fireworks.includes(char)
      )
    ) {
      validation.checks.push({
        fieldName: 'fireworks',
    ...
    if (validation.legalCompliance) {
      ...
      console.error(
        `❌ ${eventData.name} 数据验证失败:`,
        validation.checks.filter(c => !c.isAccurate).map(c => c.fieldName)
      );
    }
  ...
  validateDataFile(events: any[], region: string, month: string) {
    ...
    events.forEach(event => {
      ...
      if (validation.legalCompliance) {
        ...
        results.failed++;
        results.issues.push(
          `${event.name}: ${validation.checks
            .filter(c => !c.isAccurate)
            .map(c => c.fieldName)
            .join(', ')}`
        );
      }
  ...
  generateAccuracyReport(): string {
    ...
    this.validatedData.forEach(validation => {
      ...
      report.push(
        `- 验证状态: ${validation.legalCompliance ? '✅ 通过' : '❌ 失败'}`
      );
