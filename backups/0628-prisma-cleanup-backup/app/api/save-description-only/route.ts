...
export async function POST(request: NextRequest) {
  try {
    const { targetId, description, region, activityType } = await request.json();

    console.log('保存description请求:', { targetId, hasDescription: !!description, region, activityType });

    if (!description) {
      return NextResponse.json({
        success: false,
        error: '缺少description内容'
      }, { status: 400 });
    }
    ...
    if (targetId) {
      ...
      try {
        await prisma.$queryRawUnsafe(
          `UPDATE ${tableName} SET description = ?, updatedAt = CURRENT_TIMESTAMP WHERE id = ?`,
          description,
          targetId
        );
        ...
        console.error('更新description失败:', error);
        return NextResponse.json({
          success: false,
          error: '数据库更新失败'
        }, { status: 500 });
    ...
    if (region && activityType) {
      ...
      try {
        const records = await prisma.$queryRawUnsafe(
          `SELECT id, name, description FROM ${tableName} WHERE region = ? ORDER BY createdAt DESC LIMIT 10`,
          region
        );
    ...
    console.error('保存description失败:', error);
    return NextResponse.json({
      success: false,
      error: '服务器内部错误'
    }, { status: 500 });
