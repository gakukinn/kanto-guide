...
function buildWalkerPlusDisplay(walkerData: WalkerPlusData): string {
  ...
  // 严格按照14项顺序显示，只显示解析到的数据，不添加兜底值
  
  // 1. 大会名
  displayLines.push(`大会名\t${walkerData.eventName || ''}`);
  
  // 2. 打ち上げ数
  displayLines.push(`打ち上げ数\t${walkerData.fireworksCount || ''}`);
  
  // 3. 打ち上げ時間
  displayLines.push(`打ち上げ時間\t${walkerData.fireworksDuration || ''}`);
  
  // 4. 例年の人出
  displayLines.push(`例年の人出\t${walkerData.expectedVisitors || ''}`);
  
  // 5. 開催期間
  displayLines.push(`開催期間\t${walkerData.eventPeriod || ''}`);
  ...
  if (walkerData.eventTime) {
    ...
    displayLines.push(`開催時間\t`);
  }
  
  // 7. 荒天の場合
  displayLines.push(`荒天の場合\t${walkerData.weatherPolicy || ''}`);
  ...
  if (walkerData.paidSeats) {
    ...
    displayLines.push(`有料席\t`);
  }
  
  // 9. 屋台など
  displayLines.push(`屋台など\t${walkerData.foodStalls || ''}`);
  
  // 10. その他・全体備考
  displayLines.push(`その他・全体備考\t${walkerData.otherNotes || ''}`);
  
  // 11. 会場
  displayLines.push(`会場\t${walkerData.venue || ''}`);
  
  // 12. 会場アクセス
  displayLines.push(`会場アクセス\t${walkerData.venueAccess || ''}`);
  
  // 13. 駐車場
  displayLines.push(`駐車場\t${walkerData.parking || ''}`);
  
  // 14. 問い合わせ
  displayLines.push(`問い合わせ\t${walkerData.contactInfo || ''}`);
...
function buildWalkerFieldsArray(walkerData: WalkerPlusData): Array<{label: string, value: string}> {
  return [
    ...
    { label: '例年の人出', value: walkerData.expectedVisitors || '' },
    { label: '開催期間', value: walkerData.eventPeriod || '' },
    { label: '開催時間', value: walkerData.eventTime ? walkerData.eventTime.replace(/\n/g, ' ').trim() : '' },
    { label: '荒天の場合', value: walkerData.weatherPolicy || '' },
    { label: '有料席', value: walkerData.paidSeats ? walkerData.paidSeats.replace(/\n/g, ' ').trim() : '' },
    { label: '屋台など', value: walkerData.foodStalls || '' },
    { label: 'その他・全体備考', value: walkerData.otherNotes || '' },
    { label: '会場', value: walkerData.venue || '' },
    { label: '会場アクセス', value: walkerData.venueAccess || '' },
