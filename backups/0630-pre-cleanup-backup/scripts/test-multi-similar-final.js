...
// 🧪 多重相似活动检测最终测试 - 模拟真实场景
console.log('🧪 多重相似活动检测最终测试 - 模拟真实场景\n');

// 测试用的活动数据 - 模拟多个相似的花火大会
...
const displayConflictUI = (result) => {
  ...
  console.log('📋 相似活动列表 (用户可选择):');
  result.similarActivities.forEach((item, index) => {
    const similarity = (item.similarity * 100).toFixed(1);
    console.log(`\n   ○ 选项 ${index + 1}: ${item.activity.name}`);
    console.log(`     相似度: ${similarity}% ${similarity >= 90 ? '🔥' : similarity >= 80 ? '⚡' : '📊'}`);
...
async function runFinalTests() {
  console.log('🎯 最终测试场景: 新活动与多个现有活动高度相似\n');
