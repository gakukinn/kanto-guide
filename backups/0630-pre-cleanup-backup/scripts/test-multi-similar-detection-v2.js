...
// 🧪 多重相似活动检测测试脚本 v2 - 更准确的测试
console.log('🧪 开始测试多重相似活动检测功能 v2...\n');
...
const checkForMultipleSimilarities = (newActivity, existingActivities) => {
  ...
  existingActivities.forEach(existing => {
    ...
    if (existing.region === newActivity.region && existing.activityType === newActivity.activityType) {
      ...
      // 🆕 调整判断条件：降低阈值到75%以便测试
      const overallSimilarity = nameSimilarity >= 0.75 ? nameSimilarity : 
...
async function runTests() {
  ...
  if (result1.isDuplicate) {
    ...
    result1.similarActivities.forEach((item, index) => {
      ...
      console.log(`      查看链接: ${item.url}`);
      console.log(`      文件路径: ${item.path}\n`);
    });
    
    if (result1.similarActivities.length > 1) {
      console.log('🎯 模拟用户选择覆盖第2个活动');
      const selectedTarget = result1.similarActivities[1];
      console.log(`   选中活动ID: ${selectedTarget.id}`);
      console.log(`   将覆盖的路径: ${selectedTarget.path}\n`);
    } else {
      console.log('🎯 模拟用户选择覆盖第1个活动');
      const selectedTarget = result1.similarActivities[0];
      console.log(`   选中活动ID: ${selectedTarget.id}`);
      console.log(`   将覆盖的路径: ${selectedTarget.path}\n`);
    }
  }
