...
class DetailPagesLinkChecker {
  ...
  async checkFileLinks(filePath) {
    ...
    // 在实际使用中可以启用这个功能
    /*
    if (links.mapEmbedUrl && links.mapEmbedUrl.includes('google.com/maps/embed')) {
      const mapAccessibility = await this.checkUrlAccessibility(links.mapEmbedUrl);
    ...
    if (links.website && links.website.startsWith('http')) {
      const websiteAccessibility = await this.checkUrlAccessibility(links.website);
  ...
  printConsoleReport(report) {
    ...
    if (report.summary.errors > 0) {
      ...
      report.details.errors.forEach((error, index) => {
        console.log(`\n${index + 1}. ${error.file}`);
        console.log(`   地图链接: ${error.mapEmbedUrl || '无'}`);
        console.log(`   官方网址: ${error.website || '无'}`);
    ...
    if (report.summary.warnings > 0) {
      ...
      report.details.warnings.forEach((warning, index) => {
        ...
        console.log(`   地图链接: ${warning.mapEmbedUrl || '无'}`);
        console.log(`   官方网址: ${warning.website || '无'}`);
...
export default DetailPagesLinkChecker;
