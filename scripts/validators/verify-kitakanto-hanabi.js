...
async function extractProjectData() {
  ...
  try {
    await page.goto('http://localhost:3004/kitakanto/hanabi', { 
    ...
    const events = await page.evaluate(() => {
      ...
      cards.forEach(card => {
        ...
        if (textContent && (textContent.includes('花火') || textContent.includes('発') || textContent.includes('大会'))) {
          console.log('🎯 发现北关东花火活动:', textContent.slice(0, 100) + '...');
          ...
          const locationMatch = textContent.match(/(茨城県|栃木県|群馬県)[^📅]*?(?=📅|🎆|👥|$)/);
    ...
    console.log(`📊 项目数据抓取完成，发现 ${events.length} 个北关东花火活动`);
    return events;
...
async function extractWalkerPlusData() {
  console.log('📡 开始抓取WalkerPlus北关东花火数据...');
  ...
  const allEvents = [];
  const urls = [
    'https://hanabi.walkerplus.com/ranking/ar0310/', // 茨城
    'https://hanabi.walkerplus.com/ranking/ar0309/', // 栃木  
    'https://hanabi.walkerplus.com/ranking/ar0308/'  // 群馬
  ];
  ...
  for (const url of urls) {
    ...
    try {
      ...
      const events = await page.evaluate(() => {
        const items = document.querySelectorAll('.p-list-item, .list-item, [class*="item"], [data-testid*="item"]');
        const events = [];
        ...
        items.forEach(item => {
          ...
          if (title && title.includes('花火')) {
            ...
            // 提取详细信息
            const dateMatch = textContent.match(/(\d{4}年\d{1,2}月\d{1,2}日)/);
            const locationMatch = textContent.match(/(茨城県|栃木県|群馬県)[^期間]*?(?=期間|例年|行って|打ち上げ|$)/);
      ...
      console.log(`📡 ${url} 抓取完成，发现 ${events.length} 个活动`);
  ...
  await browser.close();
  console.log(`📡 WalkerPlus数据抓取完成，总计 ${allEvents.length} 个北关东花火活动`);
  return allEvents;
}
...
function generateReport(projectData, walkerPlusData, comparison) {
  ...
  const reportContent = `# 北关东花火数据验证报告
  
## 验证时间
${timestamp}

## 数据来源对比
- **项目数据源**: http://localhost:3004/kitakanto/hanabi
- **官方数据源**: WalkerPlus (茨城、栃木、群馬)
...
---
*此报告由Playwright+Cheerio自动生成*
`;

  const filename = `kitakanto-hanabi-verification-${timestamp.replace(/[:.]/g, '-')}.md`;
