 ...
 * 目标：https://hanami.walkerplus.com/ranking/ar0308/
 * 数据结构：三层（地区→花见会→列表）
 */
class AR0308HanamiSinglePageCrawler {
  constructor() {
    this.baseUrl = 'https://hanami.walkerplus.com/ranking/ar0308/';
    this.regionCode = 'ar0308';
    this.regionName = '茨城县';
    this.activityType = 'hanami';
    this.level = '三層';
    this.results = [];

    console.log('🌸 AR0308茨城县花见单页爬虫初始化');
  ...
  extractLocation(text) {
    ...
    for (const pattern of locationPatterns) {
      ...
      if (match) {
        ...
        // 确保包含茨城县信息
        if (!location.includes('茨城') && !location.includes('県')) {
  ...
  async crawlSinglePage() {
    ...
    try {
      ...
      console.log(`✅ 成功解析 ${results.length} 个茨城县花见景点`);
  ...
  async saveData() {
    try {
      ...
      const filename = `ar0308-hanami-single-page-${timestamp}.json`;
...
// 执行爬虫
(async () => {
  const crawler = new AR0308HanamiSinglePageCrawler();
  await crawler.run();
})();
