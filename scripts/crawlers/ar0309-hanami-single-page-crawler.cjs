 ...
 * 目标：https://hanami.walkerplus.com/ranking/ar0309/
 * 数据结构：三层（地区→花见会→列表）
 */
class AR0309HanamiSinglePageCrawler {
  constructor() {
    this.baseUrl = 'https://hanami.walkerplus.com/ranking/ar0309/';
    this.regionCode = 'ar0309';
    this.regionName = '栃木县';
    this.activityType = 'hanami';
    this.level = '三層';
    this.results = [];

    console.log('🌸 AR0309栃木县花见单页爬虫初始化');
  ...
  extractLocation(text) {
    ...
    for (const pattern of locationPatterns) {
      ...
      if (match) {
        ...
        // 确保包含栃木县信息
        if (!location.includes('栃木') && !location.includes('県')) {
          location = `栃木県・${location}`;
        }
        return location;
      }
    }
  ...
  async crawlSinglePage() {
    ...
    try {
      ...
      console.log(`✅ 成功解析 ${results.length} 个栃木县花见景点`);
  ...
  async saveData() {
    try {
      ...
      const filename = `ar0309-hanami-single-page-${timestamp}.json`;
...
// 执行爬虫
(async () => {
  const crawler = new AR0309HanamiSinglePageCrawler();
  await crawler.run();
})();
