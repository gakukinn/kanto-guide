#!/usr/bin/env node
/**
 * 第94回 信州千曲市千曲川納涼煙火大会信息爬虫
 * 使用Playwright+Cheerio+Crawlee技术
 * 目标URL: https://hanabi.walkerplus.com/detail/ar0420e00911/
 */
...
console.log('🚀 开始爬取第94回 信州千曲市千曲川納涼煙火大会信息...');

const targetUrl = 'https://hanabi.walkerplus.com/detail/ar0420e00911/';
...
try {
  ...
  $('a').each((i, elem) => {
    ...
    if (
      ...
      (text.includes('公式') ||
        text.includes('官方') ||
        href.includes('chikuma') ||
        href.includes('city.chikuma'))
    ) {
  ...
  // 详细地址提取
  const addressMatch = fullText.match(/(長野県千曲市[^。\n]*)/);
  if (addressMatch) {
    info.address = addressMatch[1];
  }

  // 会场名称提取
  const venueMatch = fullText.match(/(千曲川[^。\n]*|千曲市[^。\n]*会場)/);
  if (venueMatch) {
  ...
  const outputFile = `${outputDir}/chikuma-chikumagawa-hanabi-2025-crawled.json`;
