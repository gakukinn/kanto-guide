 ...
 *
 * 功能:
 * 1. 获取日期、时间、地点、谷歌地图、官方链接、观众数、花火数
 * 2. 保存至数据库（每个活动一个单独数据库）
 * 3. 更新四层页面信息
 */
    ...
    const crawler = new PlaywrightCrawler({
      ...
      async requestHandler({ page, request, log }) {
        ...
        try {
          ...
          // 提取页面标题
          const pageTitle = $('h1, .event-title, .detail-title')
            .first()
            .text()
            .trim();
          if (pageTitle) {
          ...
          // 提取基本信息
          $('.event-detail dt, .detail-item dt, .info-label').each(
            (i, elem) => {
              const label = $(elem).text().trim();
              const value = $(elem)
                .next('dd, .detail-value, .info-value')
                .text()
                .trim();

              switch (true) {
                case label.includes('開催日') || label.includes('日程'):
          ...
          // 提取描述信息
          const description = $(
            '.event-description, .detail-description, .info-text'
          )
            .text()
            .trim();
          if (description) {
            hanabiData.description = description;
            console.log('📄 活动描述:', description.substring(0, 100) + '...');
          }

          // 提取官方网站链接
          $('a[href*="seibu"], a[href*="官方"], a[href*="公式"]').each(
            (i, elem) => {
          ...
          // 查找谷歌地图链接
          $('a[href*="maps.google"], a[href*="goo.gl/maps"]').each(
            (i, elem) => {
              const href = $(elem).attr('href');
              if (href) {
          ...
          // 提取交通信息
          const accessInfo = $('.access-info, .traffic-info, .access')
            .text()
            .trim();
          if (accessInfo) {
            hanabiData.accessInfo = accessInfo;
          }

          // 提取停车信息
          const parkingInfo = $('.parking-info, .parking').text().trim();
          if (parkingInfo) {
            hanabiData.parkingInfo = parkingInfo;
          }

          // 保存爬取到的数据
          await this.saveToDatabase(hanabiData);
          ...
          // 更新四层页面数据文件
          await this.updatePageData(hanabiData);
...
export const seibuEnHanabiData: HanabiData = {
  ...
  _sourceData: {
    ...
    japaneseDescription: '${data.description || '西武園游乐园举办的夏季花火祭典'}',
  },
  ...
  description: '${data.description || '西武园游乐园举办的夏季限定花火祭典，连续开催期间可以在园区内欣赏到精美的花火表演，是家族游玩的理想选择。'}',
...
export const metadata: Metadata = {
  ...
  description: '${data.description || '西武园游乐园举办的夏季限定花火祭典，连续开催期间可以在园区内欣赏到精美的花火表演。'}',
...
export default SeibuEnHanabiScraper;
