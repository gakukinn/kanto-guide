 ...
 * 数据结构：三层（地区→花见会→列表）
 */
class AR0313HanamiSinglePageCrawler {
  constructor() {
    this.baseUrl = 'https://hanami.walkerplus.com/ranking/ar0313/';
    ...
    console.log('🌸 AR0313东京都花见单页爬虫初始化');
  ...
  extractLocation(text) {
    ...
    // 移除常见前缀
    let location = text.replace(/^(東京都[・・]?|都内[・・]?)/g, '').trim();
    ...
    const locationKeywords = ['区', '市', '町', '村'];
  ...
  async crawlSinglePage() {
    console.log('🚀 开始爬取AR0313东京都花见排行榜第一页数据...');
  ...
  async saveData(data) {
    ...
    const fileName = `ar0313-hanami-single-page-${timestamp}.json`;
...
// 主函数
async function main() {
  const crawler = new AR0313HanamiSinglePageCrawler();
...
module.exports = AR0313HanamiSinglePageCrawler;
