 ...
 * 目标：https://hanami.walkerplus.com/ranking/ar0312/
 * 数据结构：三层（地区→花见会→列表）
 */
class AR0312HanamiSinglePageCrawler {
  constructor() {
    this.baseUrl = 'https://hanami.walkerplus.com/ranking/ar0312/';
    this.regionCode = 'ar0312';
    this.regionName = '千叶县';
    this.activityType = 'hanami';
    this.level = '三層';
    this.results = [];

    console.log('🌸 AR0312千叶县花见单页爬虫初始化');
  ...
  extractLocation(text) {
    ...
    // 移除常见前缀
    let location = text.replace(/^(千葉県[・・]?|県内[・・]?)/g, '').trim();
  ...
  async crawlSinglePage() {
    console.log('🚀 开始爬取AR0312千叶县花见排行榜第一页数据...');
  ...
  async saveData(data) {
    ...
    const fileName = `ar0312-hanami-single-page-${timestamp}.json`;
...
// 主函数
async function main() {
  const crawler = new AR0312HanamiSinglePageCrawler();
...
module.exports = AR0312HanamiSinglePageCrawler;
