#!/usr/bin/env node
/**
 * 旭市いいおかYOU・遊フェスティバル2025 海浜花火大会信息爬虫
 * 使用Playwright+Cheerio+Crawlee技术
 * 目标URL: https://hanabi.walkerplus.com/detail/ar0312e00249/
 */
...
console.log(
  '🚀 开始爬取旭市いいおかYOU・遊フェスティバル2025 海浜花火大会信息...'
);

const targetUrl = 'https://hanabi.walkerplus.com/detail/ar0312e00249/';
...
try {
  ...
  // 详细地址提取
  const addressMatch = fullText.match(/(千葉県旭市[^。\n]*)/);
  if (addressMatch) {
  ...
  const outputFile = `${outputDir}/asahi-iioka-you-yu-hanabi-2025-crawled.json`;
  ...
  const dbFile = `${dbDir}/asahi-iioka-you-yu-hanabi-2025.db`;
  fs.writeFileSync(dbFile, JSON.stringify(database, null, 2));
  console.log(`🗃️ 数据库已创建: ${dbFile}`);

  // 生成数据库报告
  console.log(
    '\n📋 旭市いいおかYOU・遊フェスティバル2025 海浜花火大会 信息汇报'
  );
