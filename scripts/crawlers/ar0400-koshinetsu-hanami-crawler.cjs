 ...
 * 目标：https://hanami.walkerplus.com/ranking/ar0400/
 * 数据结构：三层（地区→花见会→列表）
 */
class AR0400KoshinetsuHanamiCrawler {
  constructor() {
    this.baseUrl = 'https://hanami.walkerplus.com/ranking/ar0400/';
    this.regionCode = 'ar0400';
    this.regionName = '甲信越';
    this.activityType = 'hanami';
    this.level = '三層';
    this.results = [];

    console.log('🌸 AR0400甲信越花见爬虫初始化');
  ...
  extractLocation(text) {
    ...
    for (const pattern of locationPatterns) {
      const match = text.match(pattern);
      if (match) {
        let location = this.cleanText(match[0]);
        // 如果匹配到县级信息，返回完整信息
        if (match[1] && match[2]) {
          location = `${match[1]}・${this.cleanText(match[2])}`;
        }
        return location;
      }
    }
   ...
   * 确定景点所属县份
   */
  determinePrefecture(location, name) {
  ...
  parseSpotData($, element, index) {
    try {
      ...
      // 确定所属县份
      const prefecture = this.determinePrefecture(location, name);
  ...
  async crawlSinglePage() {
    ...
    try {
      ...
      console.log(`✅ 成功解析 ${results.length} 个甲信越花见景点`);
  ...
  async saveData() {
    try {
      ...
      const filename = `ar0400-koshinetsu-hanami-${timestamp}.json`;
...
// 执行爬虫
(async () => {
  const crawler = new AR0400KoshinetsuHanamiCrawler();
  await crawler.run();
})();
