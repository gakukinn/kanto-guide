...
const checks = {
  ...
  pageStructure: {
    ...
    files: [
      ...
      ['茨城花火页面', 'src/app/ibaraki/hanabi/page.tsx'],
      ['栃木花火页面', 'src/app/tochigi/hanabi/page.tsx'],
      ['群马花火页面', 'src/app/gunma/hanabi/page.tsx'],
  
      ['新潟花火页面', 'src/app/niigata/hanabi/page.tsx'],
      ['长野花火页面', 'src/app/nagano/hanabi/page.tsx'],
      ['山梨花火页面', 'src/app/yamanashi/hanabi/page.tsx'],
    ...
    patterns: [
      ...
      },
      {
        ...
        customCheck: (content) => {
          ...
          // 检查是否在location字段中
          const locationLines = content.split('\n').filter(line => 
            line.includes('location:') && /[\u30A0-\u30FF]/.test(line)
          );
          ...
          katakanaMatches.forEach(katakana => {
            ...
            const isInLocation = locationLines.some(line => line.includes(katakana));
          ...
          return {
            ...
            details: problematicKatakana.length > 0 ? 
    ...
    rules: [
      {
        ...
        check: (content) => {
          ...
          if (title.length < 30) return { pass: false, message: `标题过短(${title.length}字符)，建议30-60字符` };
          if (title.length > 60) return { pass: false, message: `标题过长(${title.length}字符)，建议30-60字符` };
      ...
      },
      {
        ...
        check: (content) => {
          ...
          if (desc.length < 120) return { pass: false, message: `描述过短(${desc.length}字符)，建议120-160字符` };
          if (desc.length > 160) return { pass: false, message: `描述过长(${desc.length}字符)，建议120-160字符` };
    ...
    rules: [
      {
        ...
        check: (content) => {
          const issues = [];
          // 检查必要字段
          const requiredFields = ['title', 'date', 'location', 'visitors', 'fireworks'];
      ...
      },
      {
        ...
        check: (content) => {
          const hasWalkerPlus = content.includes('WalkerPlus') || content.includes('walkerplus');
          return {
            pass: hasWalkerPlus,
            message: hasWalkerPlus ? '包含WalkerPlus数据源标注' : '缺少WalkerPlus数据源标注'
          };
        }
      }
    ]
  ...
  dataAccuracy: {
    ...
    rules: [
        ...
        check: (content) => {
          ...
          if (dateMatches) {
            dateMatches.forEach(match => {
              ...
              if (dateObj.getFullYear() < currentYear || dateObj.getFullYear() > currentYear + 1) {
                issues.push(`日期年份异常: ${date}，应为${currentYear}或${currentYear + 1}年`);
              }
      ...
      },
      {
        ...
        check: (content) => {
          const issues = [];
          const locationMatches = content.match(/location:\s*['"`]([^'"`]+)['"`]/g);
          ...
          if (locationMatches) {
            locationMatches.forEach(match => {
              ...
              // 检查是否包含常见的花火大会地点关键词
              const validKeywords = ['公园', '河', '川', '海', '湖', '港', '广场', '会场', '绿地', '运动场', '体育场', '码头', '海岸', '海滨', '河岸', '河原', '桥', '大桥'];
              const hasValidKeyword = validKeywords.some(keyword => location.includes(keyword));
              
              if (!hasValidKeyword) {
                issues.push(`地点信息可能不准确: ${location}，缺少常见地点关键词`);
              }
            });
          }
        ...
        check: (content) => {
          ...
          if (visitorMatches) {
            visitorMatches.forEach(match => {
              ...
              // 检查是否包含人数信息
              if (!visitors.includes('人') && !visitors.includes('万') && !visitors.includes('千')) {
              ...
              if (numberMatch) {
                ...
                if (visitors.includes('万') && number > 100) {
                  issues.push(`观看人数可能过高: ${visitors}，超过100万人较为罕见`);
                }
                if (number < 0.1 && visitors.includes('万')) {
                  issues.push(`观看人数可能过低: ${visitors}，少于1000人使用万作单位不合理`);
                }
              }
            });
          }
          ...
          return {
            pass: issues.length === 0,
            message: issues.length === 0 ? '观看人数信息合理' : issues.join('; ')
          };
        }
      },
      {
        ...
        check: (content) => {
          ...
          if (fireworksMatches) {
            fireworksMatches.forEach(match => {
              ...
              // 检查是否包含发数信息
              if (!fireworks.includes('发') && !fireworks.includes('万') && !fireworks.includes('千') && !fireworks.includes('非公开') && !fireworks.includes('直径')) {
                issues.push(`花火数量格式异常: ${fireworks}，应包含发数或特殊说明`);
              }
              ...
              if (numberMatch) {
                ...
                if (fireworks.includes('万') && number > 10) {
                  issues.push(`花火数量可能过高: ${fireworks}，超过10万发较为罕见`);
                }
                if (number < 100 && !fireworks.includes('万') && !fireworks.includes('千')) {
                  issues.push(`花火数量可能过低: ${fireworks}，少于100发较为罕见`);
                }
              }
            });
          }
          ...
          return {
            pass: issues.length === 0,
            message: issues.length === 0 ? '花火数量信息合理' : issues.join('; ')
          };
        }
      },
      {
      ...
      },
      {
        ...
        check: (content) => {
          ...
          if (visitorMatches && fireworksMatches && visitorMatches.length === fireworksMatches.length) {
            for (let i = 0; i < visitorMatches.length; i++) {
              const visitors = visitorMatches[i].match(/['"`]([^'"`]+)['"`]/)[1];
              const fireworks = fireworksMatches[i].match(/['"`]([^'"`]+)['"`]/)[1];
              
              // 提取数字进行比较
              const visitorNum = parseFloat((visitors.match(/(\d+(?:\.\d+)?)/) || [0, 0])[1]);
              const fireworksNum = parseFloat((fireworks.match(/(\d+(?:\.\d+)?)/) || [0, 0])[1]);
              
              if (visitorNum > 0 && fireworksNum > 0) {
                // 大型活动（超过10万人）通常花火数量也较多
                if (visitors.includes('万') && visitorNum > 10 && fireworks.includes('发') && fireworksNum < 1000) {
                  issues.push(`数据不一致: ${visitors}观众但仅${fireworks}，大型活动花火数量可能偏低`);
                }
                
                // 小型活动（少于1万人）花火数量通常不会太多
                if ((!visitors.includes('万') || visitorNum < 1) && fireworks.includes('万') && fireworksNum > 1) {
                  issues.push(`数据不一致: ${visitors}观众但有${fireworks}，小型活动花火数量可能偏高`);
                }
              }
            }
          }
...
console.log(`  📊 结果: ${structurePassed}/${checks.pageStructure.files.length} 通过\n`);
    ...
    checks.codeQuality.patterns.forEach(pattern => {
      ...
      if (pattern.customCheck) {
        ...
        if (result.pass) {
          ...
          const severity = pattern.severity === 'error' ? '❌' : pattern.severity === 'warning' ? '⚠️' : 'ℹ️';
          console.log(`    ${severity} ${pattern.name}: ${result.details || pattern.message} (${result.count}处)`);
        ...
        if (matches) {
          const severity = pattern.severity === 'error' ? '❌' : pattern.severity === 'warning' ? '⚠️' : 'ℹ️';
          console.log(`    ${severity} ${pattern.name}: ${pattern.message} (${matches.length}处)`);
          if (pattern.severity === 'error') totalIssues++;
          if (pattern.severity === 'warning') qualityIssues++;
        } else {
...
if (totalIssues === 0 && accuracyIssues === 0) {
  ...
  console.log('⚠️ 基础检查通过，但发现数据真实性问题，建议核实WalkerPlus官方数据');
