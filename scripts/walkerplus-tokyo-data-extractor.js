...
async function extractTokyoHanabiData() {
  console.log('🔄 从WalkerPlus东京地区页面提取花火大会数据');
  console.log('============================================================');
  ...
  try {
    ...
    // 保存数据到JSON文件
    const outputFile = path.join(outputDir, 'tokyo-hanabi-walkerplus.json');
    fs.writeFileSync(outputFile, JSON.stringify(hanabiData, null, 2), 'utf8');

    console.log(`\n💾 数据已保存到: ${outputFile}`);

    // 创建CSV格式文件
    const csvFile = path.join(outputDir, 'tokyo-hanabi-walkerplus.csv');
    const csvHeader =
    ...
    console.log(
      `📅 有日期信息: ${stats.withDate} (${Math.round((stats.withDate / stats.total) * 100)}%)`
    );
    console.log(
      `📍 有地点信息: ${stats.withLocation} (${Math.round((stats.withLocation / stats.total) * 100)}%)`
    );
    console.log(
      `👥 有观众数信息: ${stats.withAudience} (${Math.round((stats.withAudience / stats.total) * 100)}%)`
    );
    console.log(
      `🎆 有花火数信息: ${stats.withFireworks} (${Math.round((stats.withFireworks / stats.total) * 100)}%)`
    );
...
// 执行提取
extractTokyoHanabiData()
  .then(data => {
    console.log('\n🎉 东京花火大会数据提取完成!');
    console.log(`📦 共提取 ${data.length} 个活动的信息`);
