...
// 爬取的活动数据文件路径
const DATA_FILE = path.join(__dirname, '..', 'kanagawa_events_accurate_ten_fields.json');
...
function determineEventType(eventName) {
  ...
  // 祭典活动（各种祭典、まつり）
  else if (eventName.includes('祭') || eventName.includes('まつり') || eventName.includes('パレード')) {
    return 'matsuri';
  }
  // 赏花活动（花、桜、あじさい等）
  else if (eventName.includes('花') || eventName.includes('桜') || eventName.includes('アジサイ') || 
           eventName.includes('あじさい') || eventName.includes('梅') || eventName.includes('紫陽花') || 
           eventName.includes('菊') || eventName.includes('あやめ')) {
    return 'hanami';
  }
  // 狩枫活动（紅葉、もみじ等）
  ...
  // 灯光活动（イルミネーション、ライトアップ、ぼんぼり等）
  else if (eventName.includes('イルミネーション') || eventName.includes('ライトアップ') || 
           eventName.includes('灯り') || eventName.includes('ランタン') || eventName.includes('ぼんぼり')) {
    return 'illumination';
  }
  // 默认归类为文化活动
  else {
 ...
 * 获取或创建神奈川地区记录
 */
async function getKanagawaRegion() {
  try {
    // 先尝试查找现有的神奈川地区记录
    let kanagawaRegion = await prisma.region.findFirst({
      where: {
        OR: [
          { code: 'kanagawa' },
          { nameCn: '神奈川' },
    ...
    if (!kanagawaRegion) {
      ...
      console.log('✅ 神奈川地区记录创建成功');
    } else {
      console.log('✅ 找到现有神奈川地区记录');
    }

    return kanagawaRegion;
  } catch (error) {
    console.error('❌ 获取神奈川地区记录失败:', error);
    throw error;
  }
}
 ...
 * 主要录入函数
 */
async function importKanagawaEvents() {
  console.log('🚀 开始录入神奈川县活动信息到数据库...\n');
  ...
  try {
    ...
    // 2. 获取或创建神奈川地区记录
    const kanagawaRegion = await getKanagawaRegion();
    ...
    for (let i = 0; i < eventsData.events.length; i++) {
      ...
      try {
        const operationResult = await upsertEvent(eventData, eventType, kanagawaRegion.id);
...
module.exports = { importKanagawaEvents }; 