...
export function registerHanabiDetailPage(config) {
    const existingIndex = HANABI_DETAIL_PAGES.findIndex(page => page.id === config.id);
    const newConfig = {
        ...config,
...
export function validateHanabiEvents(events) {
    ...
    events.forEach(event => {
        ...
        if (event.detailLink && !hasDetailPage) {
            warnings.push(`事件 ${event.name} (${event.id}) 设置了detailLink但没有对应的详情页面`);
            invalid.push(event);
        }
        else if (event.detailLink && event.detailLink !== detailLink) {
            warnings.push(`事件 ${event.name} (${event.id}) 的detailLink不匹配自动生成的链接`);
            invalid.push(event);
        }
        else if (hasDetailPage && !event.detailLink) {
            warnings.push(`事件 ${event.name} (${event.id}) 有详情页面但未设置detailLink`);
            invalid.push(event);
        }
        else {
            valid.push(event);
