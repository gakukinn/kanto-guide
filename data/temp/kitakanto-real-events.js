// 北关东花火真实数据（替换现有主页面数据）
const kitakantoHanabiEvents = [
  {
    id: 'moka-natsumatsuri-hanabi-2025',
    name: '真岡市夏祭大花火大会',
    japaneseName: '第53回 真岡市夏祭大花火大会',
    englishName: 'Moka Summer Festival Fireworks Display 2025',
    date: '2025年2025年7月26日',
    location: '真岡市役所东侧五行川沿岸',
    description: '在真岡市役所东侧五行川沿岸举办的花火大会，约2万发花火点亮夜空',
    features: ["北关东特色","传统花火","地域文化"],
    likes: 57,
    website: 'https://www.moka-kankou.org/',
    fireworksCount: 20000,
    expectedVisitors: 170000,
    venue: '真岡市役所东侧五行川沿岸'
  },
  {
    id: 'takasaki-hanabi-2025',
    name: '第51回高崎大花火大会',
    japaneseName: '第51回 高崎大花火大会',
    englishName: '51st Takasaki Grand Fireworks Festival',
    date: '2025年2025年8月23日（土）',
    location: '烏川和田橋上流河川敷',
    description: '在烏川和田橋上流河川敷举办的花火大会，约1万5000发花火点亮夜空',
    features: ["群马最大","传统花火","高崎特色"],
    likes: 21,
    website: 'https://www.takasaki-matsuri.jp/',
    fireworksCount: 150000000,
    expectedVisitors: 900000,
    venue: '烏川和田橋上流河川敷'
  },
  {
    id: 'ashikaga-hanabi-2025',
    name: '第109回足利花火大会',
    japaneseName: '第109回 足利花火大会',
    englishName: '109th Ashikaga Fireworks Festival',
    date: '2025年2025年8月2日（土）',
    location: '渡良瀬川河川敷主会场',
    description: '在渡良瀬川河川敷主会场举办的花火大会，约2万发花火点亮夜空',
    features: ["历史悠久","足利文化","传统祭典"],
    likes: 35,
    website: 'https://www.city.ashikaga.tochigi.jp/industory/000061/000326/p007200.html',
    fireworksCount: 20000,
    expectedVisitors: 450000,
    venue: '渡良瀬川河川敷主会场'
  }
];